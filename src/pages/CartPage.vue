...
<body>
  ...
  <main v-if="$store.state.cartLoading">Загрузка корзины...</main>
    ...
    <section class="cart">
      <form class="cart__form form" action="#" method="POST">
        ...
        <div class="cart__block">
          ...
          <router-link :disabled="$store.state.cartProductsData == 0" tag="button" :to="{name: 'order' }" class="cart__button button button--primery" type="submit">
            Оформить заказ
          </router-link>
        </div>
      </form>
...
import { mapGetters } from 'vuex';  // вспомогательная функция для проксирования геттеров
...
export default {
  ...
  components: { CartItem },
  computed: {
    ...mapGetters({products: 'cartDetailProducts', totalPrice: 'cartTotalPrice'}), // т.е запись можно сократить запись и не создавать products()
  }
}
</script>
